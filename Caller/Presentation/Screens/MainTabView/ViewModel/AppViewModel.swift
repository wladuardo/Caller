...
final class AppViewModel: ObservableObject {
    ...
    func requestFriendLocation(_ user: AppUser) async {
        do {
            try await environment.locationService.requestFriendLocation(user)
        } catch {
            callError = .general(error.localizedDescription)
        }
    }
    ...
    func setNotificationsMuted(_ isMuted: Bool, for user: AppUser) async {
        ...
        do {
            ...
            self.currentUser = AppUser(
                ...
                deviceModel: currentUser.deviceModel,
                chatEncryptionPublicKey: currentUser.chatEncryptionPublicKey
            )
        } catch {
    ...
    func openChat(with user: AppUser) {
    ...
    func takePendingInviteSearch() -> (username: String?, shouldAutoSend: Bool) {
        let payload = (pendingInviteSearchUsername, pendingInviteShouldAutoSend)
        consumePendingInviteSearch()
        return payload
    }
    ...
    func enableLocationSharing() async -> Bool {
        let granted = await environment.locationService.requestLocationAccess(for: currentUser)
        if !granted {
    ...
    private func restoreSessionIfNeeded() {
        guard let user = environment.currentUserService.currentUser else { return }
        currentUser = user
        isRestoringSession = true
    ...
    private func synchronizeCurrentUserProfile() async {
        guard let currentUser else { return }
        let currentDeviceModel = DeviceModelResolver.currentDeviceModelName()

        if let profile = try? await environment.contactService.fetchUser(id: currentUser.id) {
            let resolvedProfile = AppUser(
                id: profile.id,
                ...
                mutedNotificationUserIDs: profile.mutedNotificationUserIDs,
                deviceModel: profile.deviceModel ?? currentDeviceModel,
                sharedLocation: profile.sharedLocation,
                chatEncryptionPublicKey: profile.chatEncryptionPublicKey
            )
            self.currentUser = resolvedProfile

            if profile.deviceModel != currentDeviceModel,
               let updatedUser = try? await environment.contactService.updateDeviceModel(
                currentDeviceModel,
                for: resolvedProfile
               ) {
                self.currentUser = updatedUser
            }
        } else if currentUser.deviceModel != currentDeviceModel,
                  let updatedUser = try? await environment.contactService.updateDeviceModel(
                    currentDeviceModel,
                    for: currentUser
                  ) {
