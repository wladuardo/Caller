...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .task {
            await viewModel.trigger(.startObserving)
        }
        .onAppear {
            isComposerFocused = true
        }
        .onChange(of: viewModel.draft) { _, newValue in
            send(.draftChanged(newValue))
        }
        .onDisappear {
    ...
    private var composer: some View {
        ...
        .fileImporter(
            ...
            case .success(let urls):
                ...
                send(.setFileAttachment(url))
            case .failure:
                send(.fileAttachmentSelectionFailed)
            }
        }
    }
    ...
    private func previewAttachment(_ attachment: ChatAttachment) {
        ...
        case .file:
            guard let url = URL(string: attachment.downloadURL) else {
                send(.setErrorMessage("Не удалось открыть вложение."))
                return
            }
    ...
    private func send(_ input: ChatViewModel.Input) {
        Task {
